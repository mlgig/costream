...
def run_subject_cv(
    ...
    if len(subjects) < cv:
        raise ValueError(f"Cannot perform {cv}-fold CV with only {len(subjects)} subjects.")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(split_gen, start=1):
        ...
        for s in test_subjs:
            for df in subject_map[s]:
                # Extract raw signal
                sig = df[feature_cols].to_numpy(dtype=np.float32)
                if sig.shape[1] == 1:
                    sig = sig.flatten()
                ...
                padded = np.pad(labels, (1, 0), constant_values=0)
                events = np.where((padded[:-1] == 0) & (padded[1:] == 1))[0]
                
                if len(events) == 0:
                    test_events.append(-1)
                else:
