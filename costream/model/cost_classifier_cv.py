...
class CostClassifierCV(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, base_estimators, alpha=2, n_dirichlet=2000,
    ...
    def fit(self, X, y):
        ...
        # 1. Collect out-of-fold probabilities
        ...
        # ENSEMBLE PATH: Dirichlet Random Search
            ...
            weights_samples = rng.dirichlet(np.ones(len(self.base_estimators)), size=self.n_dirichlet)
            ...
            self.weights_ = best_w if best_w is not None else np.ones(len(self.base_estimators))/len(self.base_estimators)
        ...
        # ENSEMBLE PATH: Stacking Meta-Learner
            ...
            raise ValueError(f"Unknown method: {self.method}. Choose 'dirichlet' or 'stacking'.")

        # 3. Refit base estimators on full training set
